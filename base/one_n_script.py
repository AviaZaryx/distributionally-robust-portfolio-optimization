...
def run_1n(csv=CSV_FILE, use_custom_tol=False, custom_tol=1e-7):
    data = load_data(csv)
    if data.empty: return pd.DataFrame()

    # Define ranges for consistency with other models
    delta_range = [0, 0.005, 0.01]
    FIXED_GAMMA = 5  # Institutional benchmark standard
    ...
    for d in delta_range:
        ...
        results.append({
            'gamma_risk': FIXED_GAMMA,
            'delta': d,
            'Runtime (s)': delta_runtime,  # Added runtime here
            'mean_return': stats[0] / SCALE_FACTOR,
            'std_dev': stats[1] / SCALE_FACTOR,
